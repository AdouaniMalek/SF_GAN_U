...
# --- Paths ---
DATASET_DIR = "C:/Users/Malek Adouani/Desktop/SF_GAN_U/Dataset_Thyroid/"
...
def load_and_preprocess_data(dataset_dir: str, filename: str):
    ...
    labels = torch.from_numpy(df["Diagnosis"].to_numpy()).float().to(device)
...
train_df, COLUMN_NAMES, BINARY_INDICES, CONTINUOUS_INDICES, trainData, trainlabels = \
    load_and_preprocess_data(DATASET_DIR, "preprocessed_az.csv")
...
def compute_demographic_parity_difference(
    ...
    label_col_idx     : int         column index of the binary outcome (Diagnosis)
...
label_col_idx = COLUMN_NAMES.index("Diagnosis")
